package com.example.service.unionpay.impl;

import com.example.service.unionpay.DemoBase;
import com.example.service.unionpay.UnionPaymentService;
import com.example.util.config.SDKConfig;
import com.example.util.constant.SDKConstants;
import com.example.util.service.AcpService;
import com.example.util.unionpay.CertUtil;
...
//        response.getWriter().write(html);
    ...
    private BigDecimal getFeeAmount(BigDecimal amount, BigDecimal feeRatio, BigDecimal feeMax) {
        ...
        //最大值为feeMax
        if (null != feeMax && feeMax.compareTo(new BigDecimal("0")) >= 0) {
            fee = fee.max(feeMax);
        }
        //设置精确到分并四舍五入
